...
To build all `@plasmicpkgs/*`, run `pnpm lerna run build --scope="@plasmicpkgs/*"`.

## Workflow

1. In `plasmicpkgs-dev`, run `pnpm dev`. This should start the app host at `http://localhost:3000`.
2. Set `http://localhost:3000/plasmic-host` as the app host URL in Plasmic Studio.
3. In `@plasmicpkgs/*`, after you make a change, run `pnpm build`, then reload Plasmic Studio.
