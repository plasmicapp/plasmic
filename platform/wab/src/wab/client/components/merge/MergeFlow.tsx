...
function MergeFlow_(
  ...
  function renderReconciliation(rec: AutoReconciliation): ReactElement | null {
    ...
    const typeName = semverItem.type;
    const objName = semverItem.name;
    switch (rec.violation) {
      case "duplicate-names":
        return (
          <LineItem icon={objIcon(semverItem)}>
            <span>
              {typeName} <strong>{rec.origName}</strong>{" "}
              {isKnownComponent(rec.mergedParent) ? (
                <>
                  of Component{" "}
                  <strong>{getComponentDisplayName(rec.mergedParent)}</strong>
                </>
              ) : (
                ""
              )}{" "}
              exists in both versions, so we renamed yours to{" "}
              <strong>{rec.renamedTo}</strong>
            </span>
          </LineItem>
        );
    ...
    case "main":
      return (
        <PlasmicMergeFlow
          ...
          reconciliations={{
            children: mergeStep.autoReconciliations.map((rec) =>
