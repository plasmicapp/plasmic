...
describe("Merging virtual slots", () => {
  it("shouldn't overwrite args for unchanged slots", () => {
    ...
    function getComponentInstance(site: Site) {
      ...
      assert(
        isKnownTplComponent(instance),
        "Found instance is not of a component"
      );
    ...
    function getSlotParamContent(slot: "target" | "content") {
    ...
    for (let i = 0; i < uuidsAncestorSlotContent.length; i++) {
      expect(uuidsAncestorSlotContent[i]).not.toEqual(
        uuidsMergedSlotContent[i]
      );
    }
  });
});
