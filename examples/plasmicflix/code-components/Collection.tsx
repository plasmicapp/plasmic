  ...
  useEffect(() => {
    (async () => {
      ...
      const collectionWithExtraData = collection.map(async (movie: Movie) => {
        ...
        const certification = movieCertification.some(
          (v) => v.iso_3166_1 === "US"
        )
          ? movieCertification.find((v) => v.iso_3166_1 === "US")
        ...
        return {
          ...
          certification:
            (certification == "" ? undefined : certification) ?? "PG",
          trailers: videoData
            .slice(0, 5)
      ...
      setCollection(await Promise.all(collectionWithExtraData));
      Promise.all(collectionWithExtraData).then(() => setLoaded(true));
    })();
  }, [loaded]);
...
export const CollectionPage = (props: CollectionPageProps) => {
  ...
  const { children, columnGap, className, loading, testLoading } = props;
  const columns = 6;
