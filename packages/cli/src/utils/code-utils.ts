...
export async function fixAllImportStatements(
  ...
  try {
    await fixCssImports(context, fixImportContext);
  } catch (err) {
    logger.error(
      `Error encountered while fixing project plasmic.css @imports: ${err}`
    );
    lastError = err;
  }

  try {
    await fixNextjsGlobalCssFileImports(context);
  } catch (err) {
    logger.error(
      `Error encountered while ensuring plasmic.css imports in Next.js root file: ${err}`
    );
    lastError = err;
  }
...
// @import "../some_dep/plasmic.css"; /* plasmic-import: dep-project-id/projectcss */
const RE_CSS_IMPORT =
  /@import "[^"]*"; \/\* plasmic-import: ([^/\s]+)\/projectcss \*\//g;
...
async function fixCssImports(
  ...
  for (const project of context.config.projects) {
    if (!project.cssFilePath || !fileExists(context, project.cssFilePath)) {
      continue;
    }

    const absoluteFilePath = makeFilePath(context, project.cssFilePath);
    const prevContent = readFileText(absoluteFilePath);
    const newContent = prevContent.replace(
      RE_CSS_IMPORT,
      (sub, depProjectId) => {
        const depConfig = fixImportContext.projects[depProjectId];
        if (!depConfig?.cssFilePath) {
          return sub;
        }

        let relativePath = path.relative(
          path.dirname(absoluteFilePath),
          makeFilePath(context, depConfig.cssFilePath)
        );
        if (!relativePath.startsWith(".")) {
          relativePath = `./${relativePath}`;
        }
        return `@import "${relativePath}"; /* plasmic-import: ${depProjectId}/projectcss */`;
      }
    );

    if (prevContent !== newContent) {
      await writeFileContent(context, project.cssFilePath, newContent, {
        force: true,
      });
    }
  }
...
/**
 * Ensures the Next.js root entry file (Pages Router `_app.{ext}` or App Router
 * `layout.{ext}`) imports the project-level `plasmic.css` for every top-level
 * project. Next.js disallows global non-module CSS imports outside of these
 * files.
 *
 * Idempotent: if an import with a `plasmic-import: <projectId>/projectcss`
 * marker already exists, only its source path is updated.
 */
async function fixNextjsGlobalCssFileImports(context: PlasmicContext) {
  const config = context.config;
  if (config.platform !== "nextjs") {
    return;
  }

  const pagesDir = config.nextjsConfig?.pagesDir;
  if (pagesDir == null) {
    return;
  }

  const isAppDir = pagesDir.endsWith("app");
  const rootBasename = isAppDir ? "layout" : "_app";

  const absolutePagesDir = path.resolve(context.absoluteSrcDir, pagesDir);
  let absoluteRootPath: string | undefined;
  for (const ext of ["tsx", "jsx", "ts", "js"]) {
    const candidate = path.join(absolutePagesDir, `${rootBasename}.${ext}`);
    if (existsBuffered(candidate)) {
      absoluteRootPath = candidate;
      break;
    }
  }

  // Compute expected root path (even if file doesn't exist) for relative path calculation
  const expectedRootPath =
    absoluteRootPath || path.join(absolutePagesDir, `${rootBasename}.tsx`);

  // One import per top-level project (indirect: false). A repo can have
  // several top-level projects synced side-by-side (rare); each emits its own
  // plasmic.css and needs to be loaded globally. Transitive deps (indirect)
  // are pulled in via @import chains inside their parent's plasmic.css, so
  // we don't import them here.
  const cssImports = config.projects
    .filter(
      (p) =>
        !p.indirect && !!p.cssFilePath && fileExists(context, p.cssFilePath)
    )
    .map((p) => {
      const relativePath = path.relative(
        path.dirname(expectedRootPath),
        makeFilePath(context, p.cssFilePath)
      );
      return {
  ...
  if (!absoluteRootPath) {
    const rootFileName = isAppDir ? "app/layout.tsx" : "pages/_app.tsx";
    const importLines = cssImports
      .map(
        (c) =>
          `import "${c.importPath}"; // plasmic-import: ${c.projectId}/projectcss`
      )
      .join("\n");
    logger.warn(
      `
Could not find a Next.js root entry file (${rootFileName}). Ensure your project follows Next.js conventions.
You will need to manually add the following imports to your root file:

${importLines}

Once you add the root file, future syncs will keep the import paths up to date.`
    );
    return;
  }

  const prevContent = readFileText(absoluteRootPath);
  const newContent = await ensureNextjsGlobalCssFileImports(
    prevContent,
    cssImports,
    absoluteRootPath
  );
  if (newContent !== prevContent) {
    await writeFileContentRaw(absoluteRootPath, newContent, {
...
function isUnmarkedPlasmicCssImport(stmt: ImportDeclaration): boolean {
  const importPath = stmt.source.value;
  return (
    typeof importPath === "string" &&
    importPath.endsWith("plasmic.css") &&
    stmt.specifiers.length === 0 // side-effect import
  );
}

export async function ensureNextjsGlobalCssFileImports(
  code: string,
  cssImports: {
    projectId: string;
    importPath: string;
  ...
  const targetByProjectId = new Map(cssImports.map((c) => [c.projectId, c]));
  const handledProjectIds = new Set<string>();
  let changed = false;

  // Handle marked imports and remove unmarked plasmic.css imports
  file.program.body = file.program.body.filter((stmt) => {
    if (stmt.type !== "ImportDeclaration") {
      return true;
    }

    const spec = tryParsePlasmicImportSpec(stmt);
    if (spec && spec.type === "projectcss") {
      const target = targetByProjectId.get(spec.id);
      if (!target) {
        return true;
      }
      handledProjectIds.add(spec.id);
      if (stmt.source.value !== target.importPath) {
        stmt.source.value = target.importPath;
        changed = true;
      }
      return true;
    }

    // Remove unmarked plasmic.css imports (will be re-added with marker below)
    if (!spec && isUnmarkedPlasmicCssImport(stmt)) {
      changed = true;
      return false;
    }
  ...
  const toInsert = cssImports.filter(
    (c) => !handledProjectIds.has(c.projectId)
  );
  if (toInsert.length > 0) {
    changed = true;
    for (const { projectId, importPath } of [...toInsert].reverse()) {
      const decl = babel.types.importDeclaration(
        [],
        babel.types.stringLiteral(importPath)
      );
      babel.types.addComment(
        decl,
        "trailing",
        ` plasmic-import: ${projectId}/projectcss`,
        true
      );
      file.program.body.unshift(decl);
    }
  }
  ...
  const generated = generate(file, {
    retainLines: false,
    shouldPrintComment: () => true,
  }).code;
  return await formatAsLocal(generated, filePath, { arrowParens: "avoid" });
}
