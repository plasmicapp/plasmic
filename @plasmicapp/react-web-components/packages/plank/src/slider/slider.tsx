...
// TODO: for now, uses a hard copy of @react-aria/slider.  Will switch to the real deal
// once it has been released.

export interface PlankSliderProps extends SliderProps, StyleProps {
  getThumbLabel?: (index: number) => React.ReactNode;
  getThumbAriaLabel?: (index: number) => React.ReactNode;
}

export type PlankSliderRef = FocusableRef;

export interface PlankSliderConfig<R extends Renderer<any, any, any, any>> {
...
export function usePlankSlider<
  P extends PlankSliderProps,
  R extends AnyRenderer
>(
  plasmicClass: PlasmicClass<R>,
  props: P,
  config: PlankSliderConfig<R>,
  ref: PlankSliderRef = null
) {
  const {
    isDisabled,
    className,
